...
particle_list_pickle = pickle.load( open( 'decayed.run1.0Z.p', "rb" ) )

# assert Should check that len (particle_list) = len (particle_list_pickle)

error = 0.

for index in range (len (particle_list)) :
  for isotope in particle_list [index] :
    particle1 = particle_list [index]
    particle2 = particle_list_pickle [index]
    difference  = particle1 [isotope] - particle2 [isotope]
    mean = (particle1 [isotope] + particle2 [isotope]) / 2.0
    mean = max (1.e-16, mean)
    error = error + difference/mean
    print ('error =', isotope, difference/mean)

error = (error + difference/mean)**(1./2.) / len (particle_list)
print (error)
